...
def Alice_Load_Quantize(model_file, n_samples, num_bits):
    Alice_out, tst_msg, tst_key = Alice_output(model_file, n_samples)
    #print('Output of Alice before quantization ...\n', Alice_out)
    Alice_out_quantized = []
    ...
    return np.array(Alice_out_quantized), Alice_out_stream, Alice_out, tst_msg, tst_key
...
def Alice_output(model_file, n_samples):
    ...
    A_out = alice_model.predict([tst_msg, tst_key])

    return A_out, tst_msg, tst_key
...
if __name__ == '__main__':
    ...
    dequantized_float = dequantize_bits_to_float(quantized_bits, num_bits)
